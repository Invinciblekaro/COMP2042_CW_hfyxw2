...
import com.cw.commandLine.commands.Command;
import com.cw.commandLine.commands.HistoryEntry;
import com.cw.commandLine.commands.HistoryType;
import com.cw.commandLine.commands.PossibleCommands;
...
public class CommandLineController {
    ...
    public void init(CommandLine commandLine) {
        ...
        commandLine.getCommandBundle().setController(this);
        commandLine.getCommandBundle().setLanguageBundle(commandLine.getBundle());
    ...
    private boolean executeCommand(String[] command) {
        for (Command cmd : PossibleCommands.possibleCommands) {
            if (cmd.getKeyword().equals(command[0])) {
                cmd.execute(command, commandLine.getCommandBundle());
                return true;
            }
        }
        return false;
    ...
    private void parse() {
        ...
        if (!executeCommand(command)) {
            print(commandLine.getBundle().getString("error.unknown.command"));
        } else {
            printPrompt();
        }
    }
