...
public class Logger {
    ...
    public static void setLevel(String newLevel) {
        try {
            ...
            error(String.valueOf(newLevel) + " is no valid LogLevel - set LogLevel to NORMAL");
        }
    }


    public static void appInfo(String appName, String versionName, String versionCode, String versionDate) {
        log(LogLevel.INFO, String.valueOf(appName) + " - v" + versionName + " - (versioncode: " + versionCode + ") from " + versionDate + ")");
    }
    ...
    private static String createLogMessage(LogLevel logLevel, String message) {
        ...
        return "[" + logLevel + "] - " + format.format(Long.valueOf(System.currentTimeMillis())) + " - " + element.getClassName() + "." + element.getMethodName() + ":" + element.getLineNumber() + "  -  " + message;
    }
    ...
    public static void enableFileOutput(File folder, PrintStream outStream, PrintStream errorStream, FileOutputMode fileOutputMode) {
        ...
        try {
            System.setOut((PrintStream) new ConsoleFileStream(outLog, System.out));
            System.setErr((PrintStream) new ConsoleFileStream(errorLog, System.err));
    ...
    public static void clearLogFile() {
        ...
        try {
            File outLog = new File(folder.getAbsolutePath(), "/out.log");
            if (outLog.exists()) {
                Files.write(outLog.toPath(), "".getBytes(), new java.nio.file.OpenOption[0]);
            }
            File errorLog = new File(folder.getAbsolutePath(), "/error.log");
            if (errorLog.exists()) {
                Files.write(errorLog.toPath(), "".getBytes(), new java.nio.file.OpenOption[0]);
            }
        } catch (IOException e) {
